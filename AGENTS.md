...
- CLI output: the first line of any top-level CLI start banner should use the oracle emoji, e.g. `🧿 oracle (0.4.1) ...`. Avoid sprinkling the emoji elsewhere; only use it for the initial command headline. Exception: the TUI exit message also keeps the emoji.
