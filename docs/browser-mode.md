...
- `--browser-agent-mode <on|off|current>`: control ChatGPT Agent mode. `on` enables Agent mode via the tools menu, `off` disables it, and `current` (default) keeps the existing state.
